...
export class PendingValidationInfoDispatcher {
    ...
    public async postToChannel(payload: PendingValidationPayload): Promise<Message | null> {
        ...
        const verifyButton = new ButtonBuilder().setLabel("Verify").setStyle(ButtonStyle.Success).setCustomId("verify");
        const rejectButton = new ButtonBuilder().setLabel("Reject").setStyle(ButtonStyle.Danger).setCustomId("reject");
